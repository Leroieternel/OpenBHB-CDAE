...
def parse_arguments():
    ...
    parser.add_argument('--bs', type=int, help='num. of subjects per batch', default=16)
...
def train(train_loader, model_encoder, optimizer, opts, epoch, train_loss, params):    
    '''
    ...
    for idx, (images, ages, _) in enumerate(train_loader):   # labels: torch.Size([6])
        ...
        X = X.reshape(opts.bs * opts.sps, *X.shape[2:])    # X: (opts.bs * opts.sps, 1, 182, 218)
        ...
        # with open('ffff', 'w') as f:
            ...
            age_pred = model_encoder(torch_X)
        ...
        if idx % 50 == 0:
            ...
            # wandb.log({"original_images": wandb.Image(torch_X[0])})
...
if __name__ == '__main__':
    ...
    model_encoder = models.AlexNet3D_Dropout(num_classes=1).to(opts.device)
    ...
    # params = param_encoder + param_age_mlp
    params = param_encoder
    ...
    # training 
    train_loss = []
    wandb.init(project='3d_cnn_0608_alex_3d_1', dir='/scratch_net/murgul/jiaxia/saved_models')
    config = wandb.config
    ...
    for epoch in range(0, opts.epochs):
        ...
        train(train_loader, model_encoder, optimizer, opts, epoch, train_loss, params)
        if epoch == 29:
            checkpoint = {
                'epoch': epoch,
            ...
            torch.save(checkpoint, '/scratch_net/murgul/jiaxia/saved_models/cnn_alex_0608_1_epoch30.pth')
        if epoch == 49:
            checkpoint = {
                'epoch': epoch,
                'model_encoder_state_dict': model_encoder.state_dict(),
            ...
            torch.save(checkpoint, '/scratch_net/murgul/jiaxia/saved_models/cnn_alex_0608_1_epoch50.pth')
        ...
        if epoch == 99:
            ...
            torch.save(checkpoint, '/scratch_net/murgul/jiaxia/saved_models/cnn_alex_0608_1_epoch100.pth')
        if epoch == 149:
            checkpoint = {
                'epoch': epoch,
                'model_encoder_state_dict': model_encoder.state_dict(),
            ...
            torch.save(checkpoint, '/scratch_net/murgul/jiaxia/saved_models/cnn_0602_1_epoch150.pth')
        if epoch == 199:
            checkpoint = {
                'epoch': epoch,
                'model_encoder_state_dict': model_encoder.state_dict(),
            ...
            torch.save(checkpoint, '/scratch_net/murgul/jiaxia/saved_models/cnn_alex_0608_1_epoch200.pth')
        if epoch == 249:
            checkpoint = {
                'epoch': epoch,
                'model_encoder_state_dict': model_encoder.state_dict(),
            ...
            torch.save(checkpoint, '/scratch_net/murgul/jiaxia/saved_models/cnn_alex_0608_1_epoch250.pth')
        if epoch == 299:
            checkpoint = {
                'epoch': epoch,
                'model_encoder_state_dict': model_encoder.state_dict(),
            ...
            torch.save(checkpoint, '/scratch_net/murgul/jiaxia/saved_models/cnn_alex_0608_1_epoch300.pth')

    print('train loss: ', train_loss)
    train_loss_numpy = np.array(train_loss)
    np.save('/scratch_net/murgul/jiaxia/saved_models/cnn_alex_0608_1_loss.npy', train_loss_numpy)
    ...
    torch.save(checkpoint, '/scratch_net/murgul/jiaxia/saved_models/cnn_alex_0608_1.pth')
