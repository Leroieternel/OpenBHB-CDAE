...
def train(train_loader, model, optimizer, opts, epoch, train_loss, params):    
    ...
    for idx, (images, ages, sites) in enumerate(train_loader):   # labels: torch.Size([6])
        ...
        with torch.cuda.amp.autocast():   # automatic mixed precision
            ...
            print('torch X shape: ', torch_X.shape)      # torch.Size([32, 1, 182, 218])
            ...
            a_hat = masks_encoder_out[:, : 64]    # a^ for output of the encoder
            print('a hat shape: ', a_hat.shape)   # torch.Size([16, 64])
            zi_b_batch = torch.cat((b_batch, zi), dim=1)
            print('zi_b_batch shape: ', zi_b_batch.shape)     # torch.Size([32, 512])

            xi_b_hat = model_decoder(zi_b_batch)
            print('xi_b_hat shape: ', xi_b_hat.shape)         # torch.Size([32, 1, 182, 218])
            mask_recon = model_decoder(masks_encoder_out)
            print('mask_recon shape: ', mask_recon.shape)     # torch.Size([32, 1, 182, 218])
...
if __name__ == '__main__':
    ...
    np.save('/scratch_net/murgul/jiaxia/saved_models/cdae_60_1950_mse_bs4_0326_loss.npy', train_loss_numpy)
    ...
    torch.save(checkpoint, '/scratch_net/murgul/jiaxia/saved_models/cdae_60_1950_mse_bs4_0326.pth')
