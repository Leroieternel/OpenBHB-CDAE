...
from utils.corr_score_3d import correlation_loss_3d
...
def train(train_loader, model_encoder, model_decoder, optimizer, opts, epoch, train_loss, params, fake_label):    
    '''
    ...
    for idx, (images, ages, sites) in enumerate(train_loader):   # labels: torch.Size([6])
        ...
        # with open('ffff', 'w') as f:
            ...
            # print('age pred: ', age_pred)
            zt = z[:, : 64]
            zi = z[:, 64: ]
            # age_pred = model_age(z)
            wt = nn.Identity()
            wt_zt = wt(zt)
            wi_zi = model_wi(zi)
            ...
            xi_b_hat = model_decoder(zi_zt, x0, x1, x2, x3, x4)
            mask_recon = model_decoder(a_zi, x0, x1, x2, x3, x4)

            # age_pred = model_age(z)
            age_loss = recon_criterion(age_pred, torch_age)
            
            _, zi_hat_b_hat, x0_1, x1_1, x2_1, x3_1, x4_1 = model_encoder(xi_b_hat)
            zi_hat = zi_hat_b_hat[:, 64: ]
            zi_hat_a = torch.cat((a, zi_hat), dim=1)
            xi_a_hat = model_decoder(zi_hat_a, x0_1, x1_1, x2_1, x3_1, x4_1)

            # compute loss function
            # recon_loss = recon_criterion(mask_recon, torch_X)
            # exc_loss = ce_criterion(wt_zt, torch_sites)
            exc_loss = bce_criterion(wt_zt, a)
            inh_loss = cross_entropy_with_probs(wi_zi, fake_label)

            recon_loss = recon_criterion(mask_recon, torch_X.float())
            cycle_loss = recon_criterion(xi_a_hat, torch_X.float())
            latent_loss = recon_criterion(zi_hat, zi)
            cc_loss = correlation_loss_3d(xi_b_hat, torch_X)
            
            if epoch <= 48:
                loss = 1 * recon_loss + 8.0 * exc_loss + 0.1 * inh_loss + 0.08 * age_loss + 0.5 * cycle_loss + 4.0 * latent_loss + 1 * cc_loss

            else:
                loss = 2 * recon_loss + 8.0 * exc_loss + 0.2 * age_loss + 0.1 * inh_loss + 0.5 * cycle_loss + 4.0 * latent_loss + 1 * cc_loss
        ...
        if idx % 50 == 0:
            ...
            numpy_pred = xi_b_hat.detach().cpu().numpy()
            axial_pred = numpy_pred[:, :, :, :, 90]
            axial_pred = axial_pred.reshape(opts.bs * opts.sps, *axial_pred.shape[2:])
            print('xi_b_hat axial pred shape: ', axial_pred.shape)

            a_numpy_pred = xi_a_hat.detach().cpu().numpy()
            a_axial_pred = a_numpy_pred[:, :, :, :, 90]
            a_axial_pred = a_axial_pred.reshape(opts.bs * opts.sps, *a_axial_pred.shape[2:])
            print('xi_a_hat axial pred shape: ', a_axial_pred.shape)
            

            # print('age_pred: ', age_pred)
            # print('age loss: ', age_loss)
            # wandb.log({'epoch': epoch, 'age_loss': age_loss.item()})
            wandb.log({'epoch': epoch, 'loss': loss.item(), 'recon_loss:': recon_loss.item(), 'exc_loss': exc_loss.item(), 'inh_loss': inh_loss.item(), 'age_loss': age_loss.item(), 'cycle_loss': cycle_loss.item(), 'latent_loss': latent_loss.item(), 'cc_loss': cc_loss.item()})
            wandb.log({"original_images": wandb.Image(axial_gt[0]), "xi_b_hat_images": wandb.Image(axial_pred[0]), "xi_a_hat_images": wandb.Image(a_axial_pred[0])})
...
if __name__ == '__main__':
    ...
    # model_age = Age_Net_3d(input_dim=512)
    ...
    wandb.init(project='openbhb_cnn_0618_cdae_3d_maxpool', dir='/scratch_net/murgul/jiaxia/saved_models')
    config = wandb.config
    ...
    for epoch in range(0, opts.epochs):
        ...
        train(train_loader, model_encoder, model_decoder, optimizer, opts, epoch, train_loss, params, fake_label)
        if epoch == 29:
            checkpoint = {
                'epoch': epoch,
            ...
            torch.save(checkpoint, '/scratch_net/murgul/jiaxia/saved_models/cdae_0625_2_epoch30.pth')
        if epoch == 49:
            checkpoint = {
                'epoch': epoch,
                'model_encoder_state_dict': model_encoder.state_dict(),
            ...
            torch.save(checkpoint, '/scratch_net/murgul/jiaxia/saved_models/cdae_0625_2_epoch50.pth')
        ...
        if epoch == 99:
            ...
            torch.save(checkpoint, '/scratch_net/murgul/jiaxia/saved_models/cdae_0625_2_epoch100.pth')
        if epoch == 149:
            checkpoint = {
                'epoch': epoch,
                'model_encoder_state_dict': model_encoder.state_dict(),
            ...
            torch.save(checkpoint, '/scratch_net/murgul/jiaxia/saved_models/cdae_0625_2_epoch150.pth')
        if epoch == 199:
            checkpoint = {
                'epoch': epoch,
                'model_encoder_state_dict': model_encoder.state_dict(),
            ...
            torch.save(checkpoint, '/scratch_net/murgul/jiaxia/saved_models/cdae_0625_2_epoch200.pth')
        if epoch == 249:
            checkpoint = {
                'epoch': epoch,
                'model_encoder_state_dict': model_encoder.state_dict(),
            ...
            torch.save(checkpoint, '/scratch_net/murgul/jiaxia/saved_models/cdae_0625_2_epoch250.pth')
        if epoch == 299:
            checkpoint = {
                'epoch': epoch,
                'model_encoder_state_dict': model_encoder.state_dict(),
            ...
            torch.save(checkpoint, '/scratch_net/murgul/jiaxia/saved_models/cdae_0625_2_epoch300.pth')

    print('train loss: ', train_loss)
    train_loss_numpy = np.array(train_loss)
    np.save('/scratch_net/murgul/jiaxia/saved_models/cdae_0625_2_loss.npy', train_loss_numpy)
    ...
    torch.save(checkpoint, '/scratch_net/murgul/jiaxia/saved_models/cdae_0625_2.pth')
