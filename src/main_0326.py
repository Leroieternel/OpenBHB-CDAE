...
def read_data(image_path, label_path):    # read train: (395, 3659572)  internal val: (362, 2)  external val: (395, 2)
    ...
    x_arr = np.load(image_path, mmap_mode="r")
    
    print("- y size [original]:", y_arr.shape)
    print("- x size [original]:", x_arr.shape)
    return x_arr, y_arr

def Dataloader_quasiraw(image_path, label_path):
    x_train, y_train = read_data(image_path, label_path)
    x_train_modality = FeatureExtractor('quasiraw').transform(x_train)
    print('sshhhhhapppppe: ', x_train_modality.shape)

    return x_train_modality
...
if __name__ == '__main__':
    ...
    train_image_path = os.path.join(opts.data_dir, 'train' + ".npy")
    train_label_path = os.path.join(opts.data_dir, 'train' + ".tsv")
    internal_validation_path = os.path.join(opts.data_dir, 'internal_val' + ".tsv")
    external_validation_path = os.path.join(opts.data_dir, 'external_val' + ".tsv")
    
    x_train = Dataloader_quasiraw(train_image_path, train_label_path)
    print(x_train.shape)
    
    # set_seed(opts.trial)

    # # train_loader, test_loader_int, test_loader_ext = load_data(opts)
    # # train_x, train_y = read_data(opts)
    ...
    torch.save(checkpoint, '/scratch_net/murgul/jiaxia/saved_models/unet_1_9pm_bs4_0325.pth')
