...
def correlation_loss_3d(x_hat, x, eps=1e-8):
    # compute correlation
    x_hat_mean = torch.mean(x_hat, dim=(2, 3, 4), keepdim=True)    # [bs*sps, 1]  
    ...
    cov = torch.mean((x_hat - x_hat_mean) * (x - x_mean), dim=(2, 3, 4))
    # print('cov: ', cov)  # torch.Size([4, 1])
    
    # compute standard deviation
    x_hat_std = torch.std(x_hat, dim=(2, 3, 4), keepdim=True) + eps   # torch.Size([bs*sps, 1, 1, 1])
    # print('x_hat_std shape: ', x_hat_std.shape)      # [32, 1]
    # print('x hat std: ', x_hat_std)   
    x_std = torch.std(x, dim=(2, 3, 4), keepdim=True) + eps
