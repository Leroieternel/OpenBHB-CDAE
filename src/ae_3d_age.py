...
def train(train_loader, model_encoder, model_decoder, optimizer, opts, epoch, train_loss, params, fake_label):    
    ...
    for idx, (images, ages, sites) in enumerate(train_loader):   # labels: torch.Size([6])
        ...
        # with open('ffff', 'w') as f:
            ...
            mask_recon = model_decoder(z, x0, x1, x2, x3, x4)
            ...
            if epoch <= 48:
                loss = 1 * recon_loss + 0.08 * age_loss

            else:
                loss = 2 * recon_loss + 0.2 * age_loss
...
if __name__ == '__main__':
    ...
    wandb.init(project='openbhb_cnn_0625_alex_3d_maxpool', dir='/scratch_net/murgul/jiaxia/saved_models')
    config = wandb.config
    ...
    for epoch in range(0, opts.epochs):
        ...
        if epoch == 29:
            ...
            torch.save(checkpoint, '/scratch_net/murgul/jiaxia/saved_models/cnn_0625_2_epoch30.pth')
        if epoch == 49:
            checkpoint = {
                'epoch': epoch,
                'model_encoder_state_dict': model_encoder.state_dict(),
            ...
            torch.save(checkpoint, '/scratch_net/murgul/jiaxia/saved_models/cnn_0625_2_epoch50.pth')
        ...
        if epoch == 99:
            ...
            torch.save(checkpoint, '/scratch_net/murgul/jiaxia/saved_models/cnn_0625_2_epoch100.pth')
        if epoch == 149:
            checkpoint = {
                'epoch': epoch,
                'model_encoder_state_dict': model_encoder.state_dict(),
            ...
            torch.save(checkpoint, '/scratch_net/murgul/jiaxia/saved_models/cnn_0625_2_epoch150.pth')
        if epoch == 199:
            checkpoint = {
                'epoch': epoch,
                'model_encoder_state_dict': model_encoder.state_dict(),
            ...
            torch.save(checkpoint, '/scratch_net/murgul/jiaxia/saved_models/cnn_0625_2_epoch200.pth')
        if epoch == 249:
            checkpoint = {
                'epoch': epoch,
                'model_encoder_state_dict': model_encoder.state_dict(),
            ...
            torch.save(checkpoint, '/scratch_net/murgul/jiaxia/saved_models/cnn_0625_2_epoch250.pth')
        if epoch == 299:
            checkpoint = {
                'epoch': epoch,
                'model_encoder_state_dict': model_encoder.state_dict(),
            ...
            torch.save(checkpoint, '/scratch_net/murgul/jiaxia/saved_models/cnn_0625_2_epoch300.pth')

    print('train loss: ', train_loss)
    train_loss_numpy = np.array(train_loss)
    np.save('/scratch_net/murgul/jiaxia/saved_models/cnn_0625_2_loss.npy', train_loss_numpy)
    ...
    torch.save(checkpoint, '/scratch_net/murgul/jiaxia/saved_models/cnn_0625_2.pth')
